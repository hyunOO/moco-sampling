...
from sample import sort_loss_sequential
...
if __name__ == '__main__':
    ...
    for epoch in range(epoch_start, args.epochs + 1):
        if epoch == epoch_start + 5 and args.data_ratio < 1:
            if args.sample_method == 'random':
                total_list = list(i for i in range(len(train_data)))
                index = random.sample(total_list, int(len(train_data) * args.data_ratio))
                sampler = RandomSampler(train_data, index)
                train_loader = DataLoader(
                    train_data, batch_size=args.batch_size, shuffle=False,
                    num_workers=16, pin_memory=True, drop_last=True, sampler=sampler)

            elif args.sample_method == 'kcore':
                ...
                select_loader = DataLoader(
                    ...
                    num_workers=16, pin_memory=True)
                index = sort_loss(model, select_loader, int(len(select_dataset) * args.data_ratio), descending=True)
                ...
                select_loader = DataLoader(
                    select_dataset, batch_size=args.batch_size, shuffle=False,
                    num_workers=16, pin_memory=True)
                index = sort_loss(model, select_loader, int(len(select_dataset) * args.data_ratio), descending=False)
            ...
            elif args.sample_method == 'kcore_high_loss':
                print('Sampling method is [kcore_high_loss!]')
                select_dataset = CIFAR10(
                    root='data', train=True,
                    transform=test_transform, download=True)
                desire_len = int(len(select_dataset) * args.data_ratio)

                high_ratio = min(args.data_ratio * 1.2, 0.99)

                select_loader = DataLoader(
                    select_dataset, batch_size=20, shuffle=False,
                    num_workers=16, pin_memory=True)
                index = select_kcore(model.encoder_q, select_loader, high_ratio)
                index = np.array(index)

                select_dataset = CIFAR10Pair(
                    root='data', train=True,
                    transform=test_transform, download=True)
                next_select_sampler = RandomSampler(select_dataset, index, shuffle=False)
                select_loader = DataLoader(
                    select_dataset, batch_size=args.batch_size, shuffle=False,
                    num_workers=16, pin_memory=True, drop_last=True, sampler=next_select_sampler)
                new_index = sort_loss(model, select_loader, desire_len, descending=True)
                new_index = np.array(new_index)
 
                total_index = index[new_index]
                total_index = total_index.tolist()

                change_dataset = CIFAR10Pair(
                    root='./data', train=True, transform=train_transform, download=True)
                sampler = RandomSampler(change_dataset, total_index)
                train_loader = DataLoader(
                    change_dataset, batch_size=args.batch_size, shuffle=False,
                    num_workers=16, pin_memory=True, drop_last=True, sampler=sampler)

            elif args.sample_method == 'kcore_low_loss':
                print('Sampling method is [kcore_low_loss!]')
                select_dataset = CIFAR10(
                    root='data', train=True,
                    transform=test_transform, download=True)
                desire_len = int(len(select_dataset) * args.data_ratio)
                ...
                select_loader = DataLoader(
                    select_dataset, batch_size=args.batch_size, shuffle=False,
                    num_workers=16, pin_memory=True, drop_last=True, sampler=next_select_sampler)
                new_index = sort_loss(model, select_loader, desire_len, descending=False)
                new_index = np.array(new_index)
                ...
                index_file_name = f'./{args.sample_method}_ratio_{args.data_ratio}.txt'
